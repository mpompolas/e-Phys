...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    for ielectrode = 1:nElectrodes
        ...
        single_electrode_FFTs   = abs(all_trials_FFTs{ielectrode,2});  % CHECK IF SHOULD USE THE ABS HERE, OR THE PLOTTING_FFT DOES IT - or even the .^2
        single_electrode_trials = all_trials_FFTs{ielectrode,1};
        
        if ~isempty(single_electrode_trials)
            [FFT_of_average, ~]   = compute_FFT(sum(single_electrode_trials)./size(single_electrode_trials,1), trial.Time);  
            SFC(ielectrode, 1, :) = (abs(FFT_of_average)) ./ (sum(single_electrode_FFTs)./size(single_electrode_FFTs,1)); % CHECK IF SHOULD USE THE ABS HERE, OR THE PLOTTING_FFT DOES IT
        end
    end
